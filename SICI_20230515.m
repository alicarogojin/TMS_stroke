...
for i = unique_frames'
    ...
    if ind_min < ind_max
        CTmepLAT(pulse_ind) = CTminIND(pulse_ind); 
    else
        CTmepLAT(pulse_ind) = CTmaxIND(pulse_ind);
    end
    ...
    if state == 1
        frame_vals = {frame, state, CT_pulse_times(2)*1000, CT_pulse_times(1)*1000,...
            ...
            (CTmepLAT(1) * 1000 / sample_rate) - test_pulse * 1000}; % minus time of T pulse to get latency
    else
        frame_vals = {frame, state, CT_pulse_times(1)*1000, CT_pulse_times(2)*1000,...
            ...
            (CTmepLAT(2) * 1000 / sample_rate) - test_pulse * 1000}; % minus time of T pulse to get latency
    end
...
if and(rejection, isempty(trials_to_reject))
    ...
    mep_starts = cell2mat(trial_data(:, 8)) / 1000;
    mep_ends = cell2mat(trial_data(:, 9)) / 1000;
